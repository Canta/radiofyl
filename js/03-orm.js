...
/**
 * Clase Model.
 * Se utiliza para generalizar métodos de Model, en concordancia con la capa de ORM del servidor.
 * 
 * @author Daniel Cantarín <omega_canta@yahoo.com>
 */
app.Model = function(obj){
	...
	var f = {
		get: function(fieldname){
			if (fieldname == undefined){
				throw new Error("Clase Model, método get: se esperaba un nombre de campo.");
			}
			
			if (this._obj.datos == undefined){
				throw new Error("Clase Model, método get: formato inválido de objeto.");
			}
			var tmp = null;
			if (this._obj.datos[fieldname] !== undefined){
				//propiedad
				tmp = this._obj.datos[fieldname];
			} else if (this._obj.datos.fields[fieldname]) {
				//campo
				tmp = this._obj.datos.fields[fieldname].data.valor.join()
			} else {
				//no existe
				tmp =  "";
			}
